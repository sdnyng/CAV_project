...
def main():
    ...
    ori_img = cv2.imread('./kitti_000008.png')
    ...
    # for i in range(len(TestData)):
        ...
        cmm_det_tra_data = []
        """
        bbx data: pred_bboxes
        x in lidar coor: pred_bboxes[i_pred, 0]
        y in lidar coor: pred_bboxes[i_pred, 1]
        z in lidar coor: pred_bboxes[i_pred, 2]
        x-size
        y-size
        z-size
        yaw
        ...
        f.close()
        # print(t2)
        print('\tDone testing the {}th sample, time: {:.1f}ms, speed {:.2f}FPS'.format(i, (t5 - t1) * 1000,1 / (t5 - t1)))
        
        
        

        #x = pred_bboxes[i_pred, 0]
        #y = pred_bboxes[i_pred, 1]
        #z = pred_bboxes[i_pred, 2]
        #x_size = round(pred_bboxes[i_pred, 3], 2),
        #y_size = round(pred_bboxes[i_pred, 4], 2),
        #z_size = round(pred_bboxes[i_pred, 5], 2),
        #yaw = round(pred_bboxes[i_pred, 6], 2),
        ...
        for i in range(pred_bboxes.shape[0]):
            ...
            y = pred_bboxes[i, 1]
            z = pred_bboxes[i, 2]
            x_size = round(pred_bboxes[i, 3]/2, 2)
            y_size = round(pred_bboxes[i, 4]/2, 2)
            z_size = round(pred_bboxes[i, 5]/2, 2)
            yaw = round(pred_bboxes[i, 6], 2)
            ...
            # Translate corners to correct position
            corners[:, 0] += x
            corners[:, 1] += y
            corners[:, 2] += z

            # Create Open3D geometry object for bounding box
            lines = [
                [0, 1], [1, 2], [2, 3], [3, 0],
                [4, 5], [5, 6], [6, 7], [7, 4],
                [0, 4], [1, 5], [2, 6], [3, 7]
        ...
        # Create point cloud
        #points = o3d.io.read_point_cloud("./lidardata/"+str(t)+".bin")
